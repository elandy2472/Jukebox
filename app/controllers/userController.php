...
class userController extends mainModel
{
    ...
    public function registrarUsuarioControlador()
    {
        ...
        $nombre_usuario = $this->limpiarCadena($_POST['usuario']);
        $contraseña = $this->limpiarCadena($_POST['contraseña']);
        $empresa = $this->limpiarCadena($_POST['empresa']);
        $direccion = $this->limpiarCadena($_POST['direccion']);
        $ciudad = $this->limpiarCadena($_POST['ciudad']);
        $nit = $this->limpiarCadena($_POST['nit']);


        # Verificando campos obligatorios #
        if ($nombres == "" || $apellidos == "" || $correo == "" || $tipo_documento == "" || $nombre_usuario == "" || $contraseña == "" || $empresa == "" || $direccion == "" || $ciudad == "" || $nit == "") {
            $alerta = [
                "tipo" => "simple",
        ...
        if ($this->verificarDatos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{3,50}", $nombres)) {
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
        ...
        if ($this->verificarDatos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{3,50}", $apellidos)) {
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
        ...
        if ($correo != "") {
            if (filter_var($correo, FILTER_VALIDATE_EMAIL)) {
                $sql = "SELECT correo FROM usuarioempresa WHERE correo = :correo";
                $consulta = $this->ejecutarConsulta($sql, [':correo' => $correo]);
        ...
        if ($this->verificarDatos('(?:[1-9]|10)[0-9]{6,9}', $tipo_documento)) {
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
